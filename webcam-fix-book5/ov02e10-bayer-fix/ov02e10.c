...
//
// Experimental bayer pattern fix for OV02E10 on Samsung Galaxy Book5.
//
// Problem: When ipu-bridge reports rotation=180 for Samsung laptops,
// libcamera sets hflip=1 + vflip=1. The physical bayer pattern shifts
// but the driver never compensates, causing purple/magenta tint.
//
// Fix approach (modeled on OV02C10 kernel fix):
//   Adjust ISP window registers by 1 pixel when flips are applied,
//   shifting the CFA readout grid to preserve the bayer pattern.
//   Also removes V4L2_CTRL_FLAG_MODIFY_LAYOUT (inert but misleading).
//
// The exact registers are unknown (no public OV02E10 datasheet).
// Use the "window_offset_mode" parameter to test different candidates:
//
//   Mode 0: No window adjustment (baseline — same as MODIFY_LAYOUT-only fix)
//   Mode 1: 0x44:45 +1 on hflip, 0x42:43 +1 on vflip
//   Mode 2: 0x42:43 +1 on hflip, 0x44:45 +1 on vflip
//   Mode 3: Both start+end pairs adjusted (0x44:45/0x46:47 +1 hflip,
//           0x42:43/0x48:49 +1 vflip)
//   Mode 4: Both start+end pairs adjusted (opposite assignment from mode 3)
//
// Test: sudo modprobe -r ov02e10 && sudo modprobe ov02e10 window_offset_mode=N
//       Then open qcam and check if colors are correct.
...
static int window_offset_mode;
module_param(window_offset_mode, int, 0644);
 ...
 *   Pair B: 0x44:0x45 = 0x00:0xa8 = 168
 *   Pair C: 0x46:0x47 = 0x00:0xa8 = 168
 *   Pair D: 0x48:0x49 = 0x00:0xad = 173
 *
...
#define OV02E10_P7_REG_B_LO		CCI_REG8(0x45)
#define OV02E10_P7_REG_C_HI		CCI_REG8(0x46)
#define OV02E10_P7_REG_C_LO		CCI_REG8(0x47)
#define OV02E10_P7_REG_D_HI		CCI_REG8(0x48)
#define OV02E10_P7_REG_D_LO		CCI_REG8(0x49)

/* Base values from init array */
#define OV02E10_P7_A_BASE		0xad	/* 173 */
#define OV02E10_P7_B_BASE		0xa8	/* 168 */
#define OV02E10_P7_C_BASE		0xa8	/* 168 */
#define OV02E10_P7_D_BASE		0xad	/* 173 */
...
/*
 * Apply ISP window register adjustments to compensate for bayer pattern
 * shift caused by sensor flip. Modeled on the OV02C10 kernel fix which
 * shifts the ISP crop window by 1 pixel to preserve the bayer pattern.
 *
 * Since the OV02E10 datasheet is not public, we provide multiple modes
 * to experimentally determine which registers control the window offset.
 */
static void ov02e10_adjust_window_for_flip(struct ov02e10 *ov02e10,
					   int hflip, int vflip, int *pret)
{
	u8 a_lo, b_lo, c_lo, d_lo;

	if (window_offset_mode == 0)
		return;

	/* Start with base values from mode init array */
	a_lo = OV02E10_P7_A_BASE;	/* 0x43: 0xad = 173 */
	b_lo = OV02E10_P7_B_BASE;	/* 0x45: 0xa8 = 168 */
	c_lo = OV02E10_P7_C_BASE;	/* 0x47: 0xa8 = 168 */
	d_lo = OV02E10_P7_D_BASE;	/* 0x49: 0xad = 173 */

	switch (window_offset_mode) {
	case 1:
		/*
		 * Hypothesis: B=x_start, A=y_start (B/C are x, A/D are y)
		 * Shift x_start +1 on hflip, y_start +1 on vflip
		 */
		if (hflip)
			b_lo += 1;
		if (vflip)
	...
	case 2:
		/*
		 * Hypothesis: A=x_start, B=y_start (A/D are x, B/C are y)
		 * Opposite assignment from mode 1
		 */
		if (hflip)
			a_lo += 1;
		if (vflip)
	...
	case 3:
		/*
		 * Hypothesis: B=x_start, C=x_end, A=y_start, D=y_end
		 * Shift both start and end pairs
		 */
		if (hflip) {
			b_lo += 1;
			c_lo += 1;
	...
	case 4:
		/*
		 * Hypothesis: A=x_start, D=x_end, B=y_start, C=y_end
		 * Opposite x/y assignment from mode 3
		 */
		if (hflip) {
			a_lo += 1;
			d_lo += 1;
	...
	default:
		dev_warn_once(ov02e10->dev,
			      "ov02e10: invalid window_offset_mode %d, using 0\n",
			      window_offset_mode);
		return;
	}
	...
	cci_write(ov02e10->regmap, OV02E10_P7_REG_B_HI, 0x00, pret);
	cci_write(ov02e10->regmap, OV02E10_P7_REG_B_LO, b_lo, pret);
	cci_write(ov02e10->regmap, OV02E10_P7_REG_C_HI, 0x00, pret);
	cci_write(ov02e10->regmap, OV02E10_P7_REG_C_LO, c_lo, pret);
...
static int ov02e10_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HFLIP:
	case V4L2_CID_VFLIP:
		/* Write orientation register on page 1 */
		cci_write(ov02e10->regmap, OV02E10_REG_PAGE_FLAG,
			  OV02E10_PAGE_1, &ret);
		cci_write(ov02e10->regmap, OV02E10_REG_ORIENTATION,
			  ov02e10->hflip->val | ov02e10->vflip->val << 1, &ret);
		/*
		 * Adjust ISP window registers on page 7 to compensate for
		 * bayer pattern shift caused by flip. This is the experimental
		 * part — the correct registers depend on window_offset_mode.
		 */
		ov02e10_adjust_window_for_flip(ov02e10,
					       ov02e10->hflip->val,
					       ov02e10->vflip->val, &ret);
		break;
...
static int ov02e10_probe(struct i2c_client *client)
{
	...
	dev_info(ov02e10->dev,
		 "ov02e10: bayer-fix loaded, window_offset_mode=%d\n",
		 window_offset_mode);
...
MODULE_DESCRIPTION("OmniVision OV02E10 sensor driver (bayer pattern fix experiment)");
MODULE_LICENSE("GPL");
